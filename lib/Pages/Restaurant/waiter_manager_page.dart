...
class _WaiterManagerPageState extends State<WaiterManagerPage> {
  ...
  Future<List<WaiterDetailsModel>> fetchList() async {
    List<WaiterDetailsModel> runningOrderList = [];
    await postForWaiterDetails().then((List<WaiterDetailsModel> rList) => {
          runningOrderList.addAll(rList
              .where((element) => element.name == UserDetail.userDetails.name)),
          runningOrderList.addAll(rList
              .where((element) => element.name != UserDetail.userDetails.name))
        });
    setState(() {
